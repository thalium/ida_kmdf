...
def apply_driver_globals() -> bool:
    ...
    idc.set_name(driver_globals, "WdfDriverGlobals")
    ok = ida_typeinf.apply_cdecl(None, driver_globals, "PWDF_DRIVER_GLOBALS;", 0)
    if not ok:
...
def load_wdf(til_dir=os.path.join(idc.idadir(), "til")) -> bool:
    ...
    idc.set_name(bind_info_ea, "BindInfo")
    ok = ida_typeinf.apply_cdecl(None, bind_info_ea, "struct _WDF_BIND_INFO;", 0)
    if not ok:
